...
impl LicenseRepository for SqliteLicenseRepository {
    async fn get(&self) -> AppResult<LicenseState> {
        let state = sqlx::query_as::<_, LicenseState>(
            "SELECT id, license_key, is_activated, activated_at, last_checked_at FROM license_state WHERE id = 'license'",
        )
    ...
    async fn save(&self, license_state: &LicenseState) -> AppResult<LicenseState> {
        sqlx::query(
            r#"
            INSERT OR REPLACE INTO license_state (id, license_key, is_activated, activated_at, last_checked_at)
            VALUES (?, ?, ?, ?, ?)
            "#,
        )
