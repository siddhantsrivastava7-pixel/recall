...
/// Days a screenshot file lives on disk after capture before the GC
/// purges it. Locked to 60 in v0.2.3; a settings field lands when we
/// have actual user signal that 60 is wrong.
pub const RETENTION_DAYS: u64 = 60;
...
/// older than `RETENTION_DAYS` days and clears the `memory.url`
/// column on rows that pointed at them.
...
pub fn start_retention_loop(app: AppHandle, memory_repo: SharedMemoryRepository) {
    tauri::async_runtime::spawn(async move {
        loop {
            match run_retention_gc(&app, &memory_repo).await {
                Ok(summary) => {
