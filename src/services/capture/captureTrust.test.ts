...
import { runCaptureTrustSuite } from "@/services/capture/captureTrustRunner";
import {
...
describe("capture trust suite", () => {
  it("keeps save-to-search behavior reliable across representative capture paths", async () => {
    ...
    expect(summary.latency.maxDbWriteMs).toBeLessThanOrEqual(300);
    expect(summary.latency.maxSearchVisibleMs).toBeLessThanOrEqual(100);
    expect(summary.latency.maxFullPropagationMs).toBeLessThanOrEqual(400);
  });
});
